...
const createCache = () => {
   ...
   */
  const setCache = (utility: Utilities, data: any): void => cache[utility] = data
  return {getCache, setCache}
}

const dailyIntensityCache = createCache()
/**
...
const retriveDailyIntensity = async (
  ...
  const cache = dailyIntensityCache.getCache(utility)
  ...
  dailyIntensityCache.setCache(utility, data)
  setData(data);
};

const dailyIntensityByMonthCache = createCache()
/**
...
const retriveDailyIntensityByMonth = async (
  ...
  const cache = dailyIntensityByMonthCache.getCache(utility)
  if(cache){
    console.log(`Got Month Data for ${utility} from Local Cache`)
  ...
  dailyIntensityByMonthCache.setCache(utility, data)
  setData(data);
};

const forecastCache = createCache()
/**
...
const retriveForecast = async (
  ...
  const cache = forecastCache.getCache(utility)
  if(cache){
    console.log(`Got Forecast Data for ${utility} from Local Cache`)
    return setData(cache);
  }
  const response = await fetch(
  ...
  // TODO: Handle error responses and don't save to cache
  forecastCache.setCache(utility, data)
  setData(data);
};

/**
 * Uses JS's date object t
 * 
